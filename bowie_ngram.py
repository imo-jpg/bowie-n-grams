...
for sentence in dataset:
	...
	newSong = ['but', 'i']
	#do a while loop to go through the 'window' process until the keyname (that is, a combo of two words) doesn't exist anymore
	doneProcessing = False
	while not doneProcessing:
		newngram = newSong[len(newSong)-2:len(newSong)]
		newkeyName = newngram[0] + ' ' + newngram[1]
		newSong.append(random.choice(ngram_mapping[newkeyName]))

		#this seems to either never be true, or it just takes a long time
		if not ngram_mapping.get(newkeyName):
			doneProcessing = True
